...
new_invoice() {
  ...
  curl -s -m "$REQ_TIMEOUT" -b "$jar" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
...
CREATED=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
...
NODUE=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
...
BADDUE=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
...
TAMPER=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
...
EDITED=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/invoices/$INV" -H 'Content-Type: application/json' \
...
check "STALE_RECORD" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/invoices/$INV" \
...
check "SENT" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$INV/send" | jqp "['data']['status']")" "DRAFT -> SENT"
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/invoices/$INV" \
  -H 'Content-Type: application/json' -d '{"notes":"tampered"}' | jqp "['error']['code']")" "SENT cannot be edited"
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$INV/send" | jqp "['error']['code']")" \
  "Cannot send twice"

# A zero-value invoice must not be issued.
ZERO=$(new_invoice "$A_JAR" "$A_CUST" "Zero value" "1" "0")
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$ZERO/send" | jqp "['error']['code']")" \
...
check "VALIDATION_ERROR" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$CANC/cancel" \
  -H 'Content-Type: application/json' -d '{}' | jqp "['error']['code']")" "Cancel requires a reason"

CANCELLED=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$CANC/cancel" \
...
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$CANC/send" | jqp "['error']['code']")" \
  "Cancelled invoice cannot be sent"

# The number is retained so the sequence stays unbroken.
CANC_NUM=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/invoices/$CANC" | jqp "['data']['invoiceNumber']")
...
DELETE_CODE=$(curl -s -m "$REQ_TIMEOUT" -o /dev/null -w '%{http_code}' -b "$A_JAR" -X DELETE "$BASE/invoices/$INV")
...
PAST=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
  ...
  | jqp "['data']['id']")
curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$PAST/send" -o /dev/null

curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/recalculate-overdue" -o /dev/null
check "OVERDUE" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/invoices/$PAST" | jqp "['data']['status']")" \
  "Past-due sent invoice becomes OVERDUE"

# A draft that is past due must NOT be flagged: it was never issued.
DRAFT_PAST=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
  -d "{\"customerId\":\"$A_CUST\",\"paymentMethod\":\"BANK_TRANSFER\",\"issueDate\":\"2026-01-01\",\"dueDate\":\"2026-01-31\",
       \"items\":[{\"description\":\"Past due draft\",\"quantity\":\"1\",\"unitPrice\":\"50\"}]}" | jqp "['data']['id']")
curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/recalculate-overdue" -o /dev/null
check "DRAFT" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/invoices/$DRAFT_PAST" | jqp "['data']['status']")" \
  "Past-due DRAFT is not marked overdue"

# Running twice must not change anything further.
SECOND=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/recalculate-overdue" | jqp "['data']['updated']")
check "0" "$SECOND" "Overdue recalculation is idempotent"

check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/invoices?status=OVERDUE" | jqp "['data']['total']")" "Overdue filter works"
echo

# --- Duplicate -------------------------------------------------------------------
echo "5. Duplicate (TICKET-030)"

DUP=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices/$INV/duplicate")
...
curl -s -m "$REQ_TIMEOUT" "$PDF_URL" -o "$TMP/inv.pdf"
...
if is_uuid "$SEED_CUST"; then
  ...
  check "CANCELLED" "$(curl -s -m "$REQ_TIMEOUT" -b "$BILL_JAR" -X POST "$BASE/invoices/$BDRAFT/cancel" \
  ...
  curl -s -m "$REQ_TIMEOUT" -b "$BILL_JAR" -X POST "$BASE/invoices/$BSENT/send" -o /dev/null
  check "CANCELLED" "$(curl -s -m "$REQ_TIMEOUT" -b "$BILL_JAR" -X POST "$BASE/invoices/$BSENT/cancel" \
...
check "INVOICE_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" "$BASE/invoices/$INV" | jqp "['error']['code']")" \
  "Org B cannot read Org A's invoice"
check "INVOICE_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" -X POST "$BASE/invoices/$INV/send" | jqp "['error']['code']")" \
  "Org B cannot send Org A's invoice"
check "INVOICE_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" -X POST "$BASE/invoices/$INV/cancel" \
...
XORG_BODY=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/invoices" -H 'Content-Type: application/json' \
