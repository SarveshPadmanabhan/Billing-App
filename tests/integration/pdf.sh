...
RESP=$(curl -s -m "$REQ_TIMEOUT" -b "$JAR" "$BASE/quotations/$QID/pdf")
...
curl -s -m "$REQ_TIMEOUT" "$URL" -o "$TMP/doc.pdf"
...
BARE="${URL%%\?*}"
check "403" "$(curl -s -m "$REQ_TIMEOUT" -o /dev/null -w '%{http_code}' "$BARE")" "Unsigned URL is refused by storage"
echo
...
SECOND=$(curl -s -m "$REQ_TIMEOUT" -b "$JAR" "$BASE/quotations/$QID/pdf")
...
curl -s -m "$REQ_TIMEOUT" -b "$JAR" -X PATCH "$BASE/quotations/$QID" -H 'Content-Type: application/json' \
  -d '{"notes":"Cache-busting note that appears on the PDF"}' -o /dev/null
THIRD=$(curl -s -m "$REQ_TIMEOUT" -b "$JAR" "$BASE/quotations/$QID/pdf")
THIRD_URL=$(printf '%s' "$THIRD" | jqp "['data']['url']")
curl -s -m "$REQ_TIMEOUT" "$THIRD_URL" -o "$TMP/doc2.pdf"
...
for i in $(seq 1 10); do
  curl -s -m "$REQ_TIMEOUT" -b "$JAR" "$BASE/quotations/$CQ/pdf" > "$TMP/c_$i.json" &
done
wait
...
SEND=$(curl -s -m "$REQ_TIMEOUT" -b "$JAR" -X POST "$BASE/quotations/$SQ/send")
...
AFTER_SEND=$(curl -s -m "$REQ_TIMEOUT" -b "$JAR" "$BASE/quotations/$SQ/pdf")
...
check "401" "$(curl -s -m "$REQ_TIMEOUT" -o /dev/null -w '%{http_code}' "$BASE/quotations/$QID/pdf")" \
  "Anonymous PDF request is refused"

OTHER="$TMP/b.jar"
curl -s -m "$REQ_TIMEOUT" -c "$OTHER" -X POST "$BASE/auth/sign-up/email" -H 'Content-Type: application/json' \
  -d "{\"email\":\"pdfb-$STAMP@test.local\",\"password\":\"CorrectHorseBattery1\",\"name\":\"B U\",\"firstName\":\"B\",\"lastName\":\"U\"}" -o /dev/null
curl -s -m "$REQ_TIMEOUT" -c "$OTHER" -X POST "$BASE/auth/sign-in/email" -H 'Content-Type: application/json' \
  -d "{\"email\":\"pdfb-$STAMP@test.local\",\"password\":\"CorrectHorseBattery1\"}" -o /dev/null
curl -s -m "$REQ_TIMEOUT" -b "$OTHER" -c "$OTHER" -X POST "$BASE/organisations" -H 'Content-Type: application/json' \
  -d '{"name":"Other PDF Org","currencyCode":"INR","countryCode":"IN"}' -o /dev/null

check "QUOTATION_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$OTHER" "$BASE/quotations/$QID/pdf" | jqp "['error']['code']")" \
