...
new_customer() {
  curl -s -m "$REQ_TIMEOUT" -b "$1" -X POST "$BASE/customers" -H 'Content-Type: application/json' \
...
new_quotation() {
  ...
  curl -s -m "$REQ_TIMEOUT" -b "$jar" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
CREATED=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
TAMPER=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
EMPTY_ITEMS=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
NEG_QTY=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
BIG_DISC=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
BAD_DATES=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
XORG_BODY=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations" -H 'Content-Type: application/json' \
...
check "QUO-000003" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations/$Q2" | jqp "['data']['quotationNumber']")" \
...
check "QUO-000001" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" "$BASE/quotations/$BQ" | jqp "['data']['quotationNumber']")" \
...
EDITED=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/quotations/$Q2" -H 'Content-Type: application/json' \
...
check "STALE_RECORD" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/quotations/$Q2" -H 'Content-Type: application/json' \
...
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/accept" | jqp "['error']['code']")" \
  "DRAFT cannot be accepted directly"
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/convert-to-invoice" \
  -H 'Content-Type: application/json' -d '{}' | jqp "['error']['code']")" "DRAFT cannot be converted"

check "SENT" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/send" | jqp "['data']['status']")" "DRAFT -> SENT"
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/quotations/$QID" \
  -H 'Content-Type: application/json' -d '{"notes":"tampered"}' | jqp "['error']['code']")" "SENT cannot be edited"
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/send" | jqp "['error']['code']")" \
  "Cannot send twice"

check "ACCEPTED" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/accept" | jqp "['data']['status']")" "SENT -> ACCEPTED"
...
curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$REJ/send" -o /dev/null
check "REJECTED" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$REJ/reject" -H 'Content-Type: application/json' \
  -d '{"reason":"Too expensive"}' | jqp "['data']['status']")" "SENT -> REJECTED"
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$REJ/convert-to-invoice" \
  -H 'Content-Type: application/json' -d '{}' | jqp "['error']['code']")" "REJECTED cannot be converted"
check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$REJ/accept" | jqp "['error']['code']")" \
...
CONV=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/convert-to-invoice" \
...
check "CONVERTED" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations/$QID" | jqp "['data']['status']")" "Quotation marked CONVERTED"
check "2" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations/$QID" | jqp "len(d['data']['items'])")" "Source quotation items intact"
check "$INV_NUM" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations/$QID" | jqp "['data']['invoices'][0]['invoiceNumber']")" \
  "Quotation links to its invoice"

# Idempotency: a repeat returns the same invoice rather than making another.
REPLAY=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/convert-to-invoice" \
  -H 'Content-Type: application/json' -d '{}')
...
curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$CONC/send" -o /dev/null
curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$CONC/accept" -o /dev/null

for i in $(seq 1 8); do
  curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$CONC/convert-to-invoice" \
...
DUP=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/quotations/$QID/duplicate")
...
check "CONVERTED" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations/$QID" | jqp "['data']['status']")" "Original unchanged by duplication"
echo
...
check "2" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations?status=CONVERTED" | jqp "['data']['total']")" "Filter by status"
check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations?search=QUO-000001" | jqp "['data']['total']")" "Search by number"
check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations?status=REJECTED" | jqp "['data']['total']")" "Rejected appear under their status"
check "0" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/quotations?dateFrom=2027-01-01" | jqp "['data']['total']")" "Date filter excludes older documents"
echo

# --- Tenant isolation --------------------------------------------------------
echo "8. Tenant isolation"

check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" "$BASE/quotations" | jqp "['data']['total']")" "Org B sees only its own quotation"
check "QUOTATION_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" "$BASE/quotations/$QID" | jqp "['error']['code']")" \
  "Org B cannot read Org A's quotation by UUID"
check "QUOTATION_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" -X POST "$BASE/quotations/$QID/send" | jqp "['error']['code']")" \
  "Org B cannot send Org A's quotation"
check "QUOTATION_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" -X POST "$BASE/quotations/$QID/convert-to-invoice" \
  -H 'Content-Type: application/json' -d '{}' | jqp "['error']['code']")" "Org B cannot convert Org A's quotation"
check "QUOTATION_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" -X POST "$BASE/quotations/$QID/duplicate" | jqp "['error']['code']")" \
