...
CREATED=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers" -H 'Content-Type: application/json' \
...
IND=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers" -H 'Content-Type: application/json' \
...
check "VALIDATION_ERROR" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers" \
...
UNI=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers" -H 'Content-Type: application/json' \
...
check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?search=Alph" | jqp "['data']['total']")" \
  "Partial name match"
check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?search=alpha.test" | jqp "['data']['total']")" \
  "Email match"
check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?search=90000" | jqp "['data']['total']")" \
  "Phone match"
check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?search=ALPH" | jqp "['data']['total']")" \
  "Search is case-insensitive"
check "0" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?search=zzzznotfound" | jqp "['data']['total']")" \
  "No match returns empty"
# Alpha Client + Ravi Kumar + Ünïcode Ltd = 3 successful creates.
check "3" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers" | jqp "['data']['total']")" \
  "Empty search restores full list"

# A quoted SQL metacharacter must be treated as a literal, not executed.
check "0" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?search=%27%20OR%20%271%27%3D%271" | jqp "['data']['total']")" \
...
check "2" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?page=1&limit=2" | jqp "len(d['data']['items'])")" \
  "limit honoured"
check "2" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?page=1&limit=2" | jqp "['data']['totalPages']")" \
  "totalPages computed"
check "VALIDATION_ERROR" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?limit=5000" | jqp "['error']['code']")" \
  "Excessive limit rejected (Security Doc §33)"
check "0.0000" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?search=Alph" | jqp "['data']['items'][0]['outstanding']")" \
...
check "VALIDATION_ERROR" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers/$CUST_ID/archive" \
  -H 'Content-Type: application/json' -d '{}' | jqp "['error']['code']")" \
  "Archive requires explicit confirmation"

check "True" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers/$CUST_ID/archive" \
  -H 'Content-Type: application/json' -d '{"confirm":true,"reason":"test"}' \
  | jqp "['data']['isArchived']")" "Archive with confirm succeeds"

check "2" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers" | jqp "['data']['total']")" \
  "Archived customer leaves the default list"
check "1" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?status=archived" | jqp "['data']['total']")" \
  "Archived customer visible under status=archived"
check "3" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers?status=all" | jqp "['data']['total']")" \
  "status=all shows both"

check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/customers/$CUST_ID" \
  -H 'Content-Type: application/json' -d '{"phone":"1"}' | jqp "['error']['code']")" \
  "Archived customer cannot be edited"

check "INVALID_STATUS_TRANSITION" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers/$CUST_ID/archive" \
  -H 'Content-Type: application/json' -d '{"confirm":true}' | jqp "['error']['code']")" \
  "Double archive rejected"

check "False" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X POST "$BASE/customers/$CUST_ID/restore" \
  | jqp "['data']['isArchived']")" "Restore works"

# There must be no delete route at all.
DELETE_CODE=$(curl -s -m "$REQ_TIMEOUT" -o /dev/null -w '%{http_code}' -b "$A_JAR" -X DELETE "$BASE/customers/$CUST_ID")
...
check "STALE_RECORD" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/customers/$CUST_ID" \
...
FRESH=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers/$CUST_ID" | jqp "['data']['updatedAt']")
FRESH_BODY=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" -X PATCH "$BASE/customers/$CUST_ID" \
  -H 'Content-Type: application/json' \
...
HIST=$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers/$CUST_ID/billing-history")
...
check "0" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" "$BASE/customers" | jqp "['data']['total']")" \
  "Org B sees none of Org A's customers"
check "CUSTOMER_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" "$BASE/customers/$CUST_ID" | jqp "['error']['code']")" \
  "Org B cannot fetch Org A's customer by UUID"
check "CUSTOMER_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" "$BASE/customers/$CUST_ID/billing-history" | jqp "['error']['code']")" \
  "Org B cannot read Org A's billing history"
check "CUSTOMER_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" -X PATCH "$BASE/customers/$CUST_ID" \
  -H 'Content-Type: application/json' -d '{"phone":"666"}' | jqp "['error']['code']")" \
  "Org B cannot edit Org A's customer"
check "CUSTOMER_NOT_FOUND" "$(curl -s -m "$REQ_TIMEOUT" -b "$B_JAR" -X POST "$BASE/customers/$CUST_ID/archive" \
  -H 'Content-Type: application/json' -d '{"confirm":true}' | jqp "['error']['code']")" \
  "Org B cannot archive Org A's customer"

# Confirm the failed cross-tenant edit changed nothing.
check "222" "$(curl -s -m "$REQ_TIMEOUT" -b "$A_JAR" "$BASE/customers/$CUST_ID" | jqp "['data']['phone']")" \
