...
EMPTY=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
EMPTY_RECENT=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/recent")
...
AFTER_DRAFT=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
check "1" "$(printf '%s' "$AFTER_DRAFT" | jqp "len(d['data']['invoices'])" <<< "$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/recent")")" \
...
curl -s -m 60 -b "$JAR" -X POST "$BASE/invoices/$DRAFT/send" -o /dev/null
SENT=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
curl -s -m 30 -b "$JAR" -X POST "$BASE/invoices/$DRAFT/payments" -H 'Content-Type: application/json' \
  -d "{\"amount\":\"2000\",\"paymentDate\":\"$TODAY\",\"paymentMethod\":\"CASH\",\"idempotencyKey\":\"dash-part-$STAMP\"}" -o /dev/null

PART=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
FULL=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
OD=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
curl -s -m 30 -b "$JAR" -X POST "$BASE/invoices/$OLD_PAID/payments" -H 'Content-Type: application/json' \
  -d "{\"amount\":\"1000\",\"paymentDate\":\"$TODAY\",\"paymentMethod\":\"CASH\",\"idempotencyKey\":\"dash-old-$STAMP\"}" -o /dev/null

OD2=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
BEFORE_CANCEL=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary" | jqp "['data']['totals']['totalInvoiced']")
curl -s -m 20 -b "$JAR" -X POST "$BASE/invoices/$CANC/cancel" -H 'Content-Type: application/json' \
  -d '{"reason":"Dashboard test cancellation"}' -o /dev/null
AFTER_CANCEL=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary" | jqp "['data']['totals']['totalInvoiced']")
...
SUM=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary")
...
LIST_OUT=$(curl -s -m 15 -b "$JAR" "$BASE/invoices?outstanding=true&limit=100" | python3 -c "
import sys,json
d=json.load(sys.stdin)['data']
print(f\"{sum(float(i['amountDue']) for i in d['items']):.2f}\")" 2>/dev/null)
LIST_COUNT=$(curl -s -m 15 -b "$JAR" "$BASE/invoices?outstanding=true&limit=100" | jqp "['data']['total']")
...
PAY_TOTAL=$(curl -s -m 15 -b "$JAR" "$BASE/payments?status=RECORDED&limit=100" | python3 -c "
import sys,json
d=json.load(sys.stdin)['data']
...
RECENT=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/recent")
...
PIPE=$(curl -s -m 15 -b "$JAR" "$BASE/dashboard/summary" | jqp "['data']['quotationPipeline']['openValue']")
...
B_JAR="$TMP/b.jar"
curl -s -m 15 -c "$B_JAR" -X POST "$BASE/auth/sign-up/email" -H 'Content-Type: application/json' \
  -d "{\"email\":\"dashb-$STAMP@test.local\",\"password\":\"CorrectHorseBattery1\",\"name\":\"D B\",\"firstName\":\"D\",\"lastName\":\"B\"}" -o /dev/null
curl -s -m 15 -c "$B_JAR" -X POST "$BASE/auth/sign-in/email" -H 'Content-Type: application/json' \
  -d "{\"email\":\"dashb-$STAMP@test.local\",\"password\":\"CorrectHorseBattery1\"}" -o /dev/null
curl -s -m 15 -b "$B_JAR" -c "$B_JAR" -X POST "$BASE/organisations" -H 'Content-Type: application/json' \
  -d '{"name":"Other Dashboard Org","currencyCode":"INR","countryCode":"IN"}' -o /dev/null

B_SUM=$(curl -s -m 15 -b "$B_JAR" "$BASE/dashboard/summary")
...
check "0" "$(curl -s -m 15 -b "$B_JAR" "$BASE/dashboard/recent" | jqp "len(d['data']['invoices'])")" \
  "And none of its recent documents"

check "401" "$(curl -s -m 10 -o /dev/null -w '%{http_code}' "$BASE/dashboard/summary")" \
